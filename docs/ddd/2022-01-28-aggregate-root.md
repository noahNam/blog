...
## 애그리거트 루트
![Aggregate](./../../static/img/ddd/Aggregate.png)

[Aggregate Post 링크](./ddd-aggregate)
...
![Aggregate Root Entity](./../../static/img/ddd/애그리거트루트.png)
...
- <strong><u>불필요한 중복을 피하고 애그리거트 루트를 통해서만 도메인 로직을 구현하는 것이 좋은데 이를 위해 2가지를 꼭 지키는 것이 좋다.</u></strong>
  ...
  - <strong>밸류 타입은 불변으로 구현한다.</strong>
     ...
     - 아래 코드를 보면 애그리거트 루트가 제공하는 `change_shipping_info` 함수에 새로운 밸류객체를 전달해서 값을 변경하는 방법밖에 없다.
